...
contract TaskManagerUpgradeable is AccessControlUpgradeable {
     ...
     * Only callable by accounts with the PARTNER_ROLE.
     */
    function createPartner() public onlyRole(PARTNER_ROLE) {
        require(!partners.contains(_partner), "Partner already exists");
     ...
     * Only callable by accounts with the PARTNER_ROLE.
     */
    function removePartner(address _partiner) public onlyRole(PARTNER_ROLE) {
    ...
    function receiveFunds(
        ...
        require(tasks[_taskId].fulfilledTime == 0, "Task is fulfilled");
     ...
     * Only callable by accounts with the PARTNER_ROLE.
     */
    function forceBurn(uint256 _taskId) public onlyRole(PARTNER_ROLE) {
        require(tasks[_taskId].state == 2, "Invalid state");
        tasks[_taskId].state = 3; // Task state is set to 'burned'
        IPartner(tasks[_taskId].partner).burn(tasks[_taskId].amount);
        emit Burned(_taskId);
